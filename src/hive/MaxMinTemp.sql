...
INSERT OVERWRITE DIRECTORY 's3://dhotrem/hive/MaxTotalOutput/' 
select stn, year, max(cast(maxtemp as FLOAT)) 
from Weather where maxtemp!=9999.9 group by stn, year;


INSERT OVERWRITE DIRECTORY 's3://dhotrem/hive/MinTotalOutput/' 
select stn, year, min(cast(mintemp as FLOAT)) 
from Weather where mintemp!=9999.9 group by stn, year;
